import 'package:flutter/material.dart';

const primaryColor = Color(0xFF4461F2);

/* LIGHT MODE COLORS */
const lightBackgroundColor = Color(0xFFF6F6F6);
const lightSecondaryTextColor = Color(0xFF757575);
const lightSuccessColor = Color(0xFF388E3C);
const lightErrorColor = Color(0xFFD32F2F);

const lightInputFieldBackgroundColor = Colors.white;
const lightInputFieldBorderColor = Color(0xFFEEEDED);
const lightInputFieldTextColor = Color(0xFF757575);

/* DARK MODE COLORS */
const darkBackgroundColor = Color(0xFF232323);
const darkSecondaryTextColor = Color(0xFFBABABA);
const darkSuccessColor = Color(0xFF66BB6A);
const darkErrorColor = Color(0xFFF44336);

const darkInputFieldBackgroundColor = Color(0xFF232323);
const darkInputFieldBorderColor = Color(0xFF6B6B6B);
const darkInputFieldTextColor = Color(0xFFC7C7C7);
